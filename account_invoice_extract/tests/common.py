# -*- coding: utf-8 -*-
# Part of Odoo. See LICENSE file for full copyright and licensing details.

from contextlib import contextmanager
from unittest.mock import patch

from odoo.addons.account_invoice_extract.models.account_invoice import AccountMove
from odoo.addons.iap.models.iap_account import IapAccount
from odoo.addons.partner_autocomplete.models.iap_autocomplete_api import IapAutocompleteEnrichAPI
from odoo.sql_db import Cursor
from odoo.tests import common


class MockIAP(common.BaseCase):
    @contextmanager
    def mock_iap_extract(self, extract_response, partner_autocomplete_response):
        def _contact_iap_extract(local_endpoint, params):
            return extract_response

        def _contact_iap_partner_autocomplete(local_endpoint, action, params, timeout=15):
            return partner_autocomplete_response

        def get_credits(service_name):
            return 1

        def commit():
            pass

        try:
            with patch.object(AccountMove, '_contact_iap_extract', side_effect=_contact_iap_extract):
                with patch.object(IapAutocompleteEnrichAPI, '_contact_iap', side_effect=_contact_iap_partner_autocomplete):
                    # the module iap is committing the transaction when creating an IAP account, we mock it to avoid that
                    with patch.object(IapAccount, 'get_credits', side_effect=get_credits):
                        with patch.object(Cursor, 'commit', side_effect=commit):
                            yield
        finally:
            pass
