...
class AccountBankReconciliationReport(models.Model):
    _inherit = "account.report"

    def _custom_options_initializer_bank_reconciliation_report(self, options, previous_options=None):
        ...
        is_multi_currency = self.user_has_groups('base.group_multi_currency') and self.user_has_groups('base.group_no_one')
    ...
    def _bank_reconciliation_report_get_inconsistent_statements(self, options, journal):
    ...
    def _bank_reconciliation_report_get_bank_miscellaneous_move_lines_domain(self, options, journal):
    ...
    def open_bank_miscellaneous_move_lines(self, options, params):
        ...
        return {
            ...
            'domain': self._bank_reconciliation_report_get_bank_miscellaneous_move_lines_domain(options, journal),
        }
    ...
    def _build_section_report_lines(self, options, journal, unfolded_lines, totals, title, title_hover):
        ...
        section_report_line = {
            ...
            'columns': [
                {
                    'name': self.format_value(totals.get(column['column_group_key']), currency=report_currency, figure_type='monetary'),
    ...
    def _get_statement_report_lines(self, options, journal):
        ...
        grouped_results = {}
        query, params = self._bank_reconciliation_report_get_statement_lines_query(options, journal)
        ...
        for st_line_id, column_group_results in grouped_results.items():
            ...
            for column in options['columns']:
                ...
                if col_expr_label == 'label':
                    col_value = results and self._format_aml_name(results['payment_ref'], results['ref'], '/') or None
                else:
                ...
                if col_value is None:
                    ...
                    if col_expr_label == 'amount_currency':
                        ...
                        formatted_value = self.format_value(col_value, currency=foreign_currency, figure_type=column['figure_type'])
                    elif col_expr_label == 'amount':
                        col_value *= reconcile_rate
                        col_class = 'number'
                        formatted_value = self.format_value(col_value, currency=report_currency, figure_type=column['figure_type'])
        ...
        return (
            self._build_section_report_lines(options, journal, plus_report_lines, plus_totals,
            ...
            self._build_section_report_lines(options, journal, less_report_lines, less_totals,
    ...
    def _bank_reconciliation_report_get_statement_lines_query(self, options, journal):
        queries = []
        params = []
        for column_group_key, column_group_options in self._split_options_per_column_group(options).items():

            tables, where_clause, where_params = self._query_get(column_group_options, 'strict_range', domain=[
                ('journal_id', '=', journal.id),
                ('account_id', '!=', journal.default_account_id.id),
            ])
    ...
    def _get_payment_report_lines(self, options, journal):
        ...
        grouped_results = {}
        query, params = self._bank_reconciliation_report_get_payment_query(options, accounts, journal)
        self._cr.execute(query, params)
            ...
            for column in options['columns']:
                ...
                if col_value is None:
                    ...
                    if col_expr_label == 'amount_currency':
                        ...
                        col_class = 'number'
                        formatted_value = self.format_value(col_value, currency=journal_currency, figure_type=column['figure_type'])
                        line_amounts[column['column_group_key']] += col_value
                        if col_value >= 0:
                            plus_totals[column['column_group_key']] += col_value
                        else:
    ...
    def _bank_reconciliation_report_get_payment_query(self, options, accounts, journal):
        queries = []
        params = []
        for column_group_key, column_group_options in self._split_options_per_column_group(options).items():

            tables, where_clause, where_params = self._query_get(column_group_options, 'normal', domain=[
                ('journal_id', '=', journal.id),
    ...
    def _dynamic_lines_generator_bank_reconciliation(self, options, all_column_groups_expression_totals):
        ...
        options['inconsistent_statement_ids'] = self._bank_reconciliation_report_get_inconsistent_statements(options, journal).ids
        ...
        column_values = {}
        for column_group_key, column_group_options in self._split_options_per_column_group(options).items():
            domain = self._get_options_domain(column_group_options, 'normal')
            ...
            balance_cells[column_group_key] = {
                'name': self.format_value(balance_gl, currency=report_currency, figure_type='monetary'),
                'balance': balance_gl,
                'class': 'number',
            }
            if last_statement:
                    ...
                    balance_cells[column_group_key].update({
                        ...
                        'title': _(
                            ...
                            "to an unexplained difference of %s.",
                             ...
                             self.format_value(balance_end, currency=report_currency, figure_type='monetary'),
                             self.format_value(difference, currency=report_currency, figure_type='monetary'),
                         ),
                    })
        ...
        balance_gl_report_line = {
            'id': self._get_generic_line_id(None, None, markup='balance_gl_line'),
        ...
        if self.env.company.totals_below_sections:
            section_st_report_lines.append((0, {
                'id': self._get_generic_line_id(None, None, markup='total', parent_line_id=balance_gl_report_line['id']),
        ...
        if plus_pay_lines or less_pay_lines:
            ...
            outstanding_payments_report_line = {
                'id': self._get_generic_line_id(None, None, markup='outstanding_payments'),
            ...
            if self.env.company.totals_below_sections:
                section_pay_report_lines.append((0, {
                    'id': self._get_generic_line_id(None, None, markup='total', parent_line_id=outstanding_payments_report_line['id']),
