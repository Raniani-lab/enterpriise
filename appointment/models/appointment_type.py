...
class AppointmentType(models.Model):
    ...
    def _slots_available(self, slots, start_dt, end_dt, staff_user=None):
        ...
        :param <res.users> staff_user: filter available slots for those users (can be a singleton
    ...
    def _get_appointment_slots(self, timezone, staff_user=None, reference_date=None):
        ...
        if not slots:
            return slots
        valid_users = staff_user.filtered(lambda user: user in self.staff_user_ids) if staff_user else None
        # Not found staff user : incorrect configuration -> skip useless computation
        if staff_user and not valid_users:
            return []
