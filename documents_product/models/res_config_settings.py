...
class ResConfigSettings(models.TransientModel):
    ...
    def _get_default_product_folder(self):
        folder_id = self.env.company_id.product_folder
        if folder_id.exists():
            return folder_id
        return False

    documents_product_settings = fields.Boolean(related='company_id.documents_product_settings', readonly=False,
                                                default=lambda self: self.env.company_id.documents_product_settings,
                                                string="Product")
    ...
    product_tags = fields.Many2many('documents.tag', 'product_tags_table',
                                    ...
                                    default=lambda self: self.env.company_id.product_tags.ids,
                                    string="Product Tags")
