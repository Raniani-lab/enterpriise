...
class ResConfigSettings(models.TransientModel):
    ...
    def _get_default_project_folder(self):
        folder_id = self.env.company.project_folder
        if folder_id.exists():
            return folder_id
        return False

    documents_project_settings = fields.Boolean(related='company_id.documents_project_settings', readonly=False,
                                                default=lambda self: self.env.company.documents_project_settings,
                                                string="Project")
    ...
    project_tags = fields.Many2many('documents.tag', 'project_tags_table',
                                    ...
                                    default=lambda self: self.env.company.project_tags.ids,
                                    string="Project Tags")
