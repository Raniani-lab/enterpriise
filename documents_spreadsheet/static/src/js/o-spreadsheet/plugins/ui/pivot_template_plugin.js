odoo.define("documents_spreadsheet.PivotTemplatePlugin", function (require) {
    ...
    class PivotTemplatePlugin extends spreadsheet.UIPlugin {
         ...
         */
        _getCells(regex) {
            return this.getters.getSheets()
                .map((sheet) => Object.values(this.getters.getCells(sheet.id))
                    .filter((cell) => regex.test(this.getters.getFormulaCellContent(sheet.id, cell))))
                .flat()
        }
        ...
        _pivotPositionToAbsolute(ast) {
            ...
            const id = this.getters.getPivot(pivotId).cache.getFieldValues(fieldName)[position - 1];
            return {
        ...
        _pivotHeader_absoluteToRelative(ast) {
            ...
            ast.args = [
                pivotIdAst,
                ...this._domainToRelative(pivotIdAst, domainAsts),
            ];
            return ast;
        }
        /**
        ...
        _domainToRelative(pivotIdAst, domainAsts) {
            ...
            for (let i = 0; i <= domainAsts.length - 1; i += 2) {
                ...
                const fieldName = JSON.parse(fieldAst.value);
                if (
                    this._isAbsolute(fieldName, pivot) &&
        ...
        _removeInvalidPivotRows() {
            for (let sheet of this.getters.getSheets()) {
                ...
                this.dispatch("REMOVE_ROWS", { rows: invalidRows, sheetId: sheet.id });
            }
            this._convertFormulas(
                this._getCells(/^\s*=.*PIVOT.*#IDNOTFOUND/),
                () => null,
            );
        }
