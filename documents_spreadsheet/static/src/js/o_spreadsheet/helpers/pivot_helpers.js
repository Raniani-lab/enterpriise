...
export async function createPivotCache(pivot, rpc, oldCache = undefined, { initialDomain = false } = {}) {
    ...
    return await _createCache(resultGB, resultFG, resultSR, pivot, rpc);
}
 ...
 */
export function formatGroupBy(cache, gp) {
    let [name, period] = gp.split(":");
    period = periods[period];
    return cache.getField(name).string + (period ? ` (${period})` : "");
}
/**
 ...
 */
export function formatHeader(cache, groupBy, value) {
    if (groupBy === "measure") {
        if (value === "__count") {
            return _t("Count");
        }
    ...
    if (["date", "datetime"].includes(cache.getField(groupBy.split(":")[0]).type)) {
        return formatDate(groupBy, value);
    }
    return cache.getGroupLabel(groupBy, value);
}
/**
 ...
 */
export async function waitForIdle(getters) {
