...
import { createView, dom, fields, nextTick } from "web.test_utils";
...
module(
  ...
  function () {
    ...
    test("Sync status is correctly rendered", async function (assert) {
      assert.expect(3);
      const { webClient, model, transportService } =
    ...
    test("input width changes when content changes", async function (assert) {
      ...
      assert.ok(
        ...
        "It should have grown to fit content"
      );
      width = input.offsetWidth;
      await fields.editInput(input, "");
      assert.ok(
        width < input.offsetWidth,
        "It should have the size of the placeholder text"
      );
    });
    ...
    test("trailing white spaces are trimmed", async function (assert) {
      ...
      assert.equal(
        input.value,
        "My spreadsheet",
        "It should not have trailing white spaces"
      );
    ...
    test("only white spaces show the placeholder", async function (assert) {
      ...
      assert.ok(
        width < input.offsetWidth,
        "It should have the placeholder size"
      );
    });
        ...
        mockRPC: async function (route, args) {
          if (
            ...
            assert.deepEqual(
              args.args[0],
              [1],
              "It should write the correct document"
            );
            return true;
          }
    ...
    test("relational PIVOT.HEADER with undefined id", async function (assert) {
      ...
      assert.equal(
        model.getters.getCell(sheetId, 0, 24),
        null,
    ...
    test("Reinsert a pivot", async function (assert) {
      ...
      const root = cellMenuRegistry
        .getAll()
        .find((item) => item.id === "reinsert_pivot");
    ...
    test("Keep applying filter when pivot is re-inserted", async function (assert) {
      ...
      assert.equal(
        ...
        "",
        "The value should have been filtered"
      );
      assert.equal(
      ...
      assert.equal(
        getCellValue(model, "B3"),
        "",
        "The value should still be filtered"
      );
      assert.equal(
        getCellValue(model, "C3"),
        "",
    ...
    test("Open pivot properties properties", async function (assert) {
      ...
      const pivotA3 = await model.getters.getPivotFromPosition(0, 2);
      ...
      const pivotA1 = await model.getters.getPivotFromPosition(0, 0);
    ...
    test("Verify presence of pivots in top menu bar in a spreadsheet with a pivot", async function (assert) {
      ...
      assert.ok(
        $(webClient.el).find("div[data-id='pivots']")[0],
        "The 'Pivots' menu should be in the dom"
      );

      const root = topbarMenuRegistry
        .getAll()
        .find((item) => item.id === "pivots");
      const children = topbarMenuRegistry.getChildren(root, env);
      assert.equal(
        children.length,
        5,
        "There should be 5 children in the menu"
      );
    ...
    test("Pivot focus changes on top bar menu click", async function (assert) {
      ...
      env.openSidePanel("PIVOT_PROPERTIES_PANEL", {});
      assert.notOk(
        model.getters.getSelectedPivot(),
        "No pivot should be selected"
      );
      children[0].action(env);
      assert.equal(
        model.getters.getSelectedPivot(),
        model.getters.getPivot(1),
        "The selected pivot should have id 1"
      );
      children[1].action(env);
      assert.equal(
        model.getters.getSelectedPivot(),
        model.getters.getPivot(2),
    ...
    test("Pivot focus changes on sidepanel click", async function (assert) {
      ...
      assert.notOk(
        model.getters.getSelectedPivot(),
        "No pivot should be selected anymore"
      );
      assert.containsN(webClient, ".o_side_panel_select", 2);
    ...
    test("Simple display", async function (assert) {
      ...
      assert.notOk(
        $(webClient.el).find(".o_spreadsheet_global_filters_side_panel")[0]
      );
    ...
    test("Display with an existing 'Date' global filter", async function (assert) {
      ...
      assert.ok(
        $(webClient.el).find(".o_spreadsheet_filter_editor_side_panel")
      );
      assert.equal(
        $(webClient.el).find(".o_global_filter_label")[0].value,
        label
      );
    });
    ...
    test("Create a new global filter", async function (assert) {
      ...
      await dom.click(
        $(webClient.el).find(".o_field_selector_select_button")[0]
      );
    ...
    test("Create a new relational global filter", async function (assert) {
      ...
      const searchIcon = $(webClient.el).find(".o_topbar_filter_icon")[0];
      await dom.click(searchIcon);
      const newRelation = $(webClient.el).find(
        ".o_global_filter_new_relation"
      )[0];
      ...
      assert.deepEqual(globalFilter.fields[1], {
        field: "product",
        type: "many2one",
      });
    });
    ...
    QUnit.test(
      "Display with an existing 'Relation' global filter",
      async function (assert) {
        assert.expect(8);
        ...
        assert.equal(items.length, 2);
        const labelElement = items[0].querySelector(
          ".o_side_panel_filter_label"
        );
        assert.equal(labelElement.innerText, label);
        await dom.click(items[0].querySelector(".o_side_panel_filter_icon"));
        assert.ok(
        ...
        assert.equal(
          webClient.el.querySelector(".o_global_filter_label").value,
          label
        );
        assert.equal(
          webClient.el.querySelector(`.o_field_many2one[name="ir.model"] input`)
            .value,
          "Product"
        );
    ...
    test("Open pivot dialog and insert a value, with UNDO/REDO", async function (assert) {
      ...
      const root = cellMenuRegistry
        .getAll()
        .find((item) => item.id === "insert_pivot_cell");
      ...
      await dom.click(
        document.body.querySelectorAll(".o_pivot_table_dialog tr th")[1]
      );
