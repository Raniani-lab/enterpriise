...
class TestHrReferral(TestHrReferralBase):
    ...
    def test_referral_multi_company(self):
        # Create 2 applicants: for the same user but with 2 differents employee.
        job_applicant = self.env['hr.applicant'].create({
            'name': 'Technical worker',
            'description': 'A nice applicant !',
            'job_id': self.job_dev.id,
            'source_id': self.richard_emp.utm_source_id.id
        })
        job_applicant_2 = self.env['hr.applicant'].create({
            'name': 'Technical worker 2',
            'description': 'A nice applicant !',
            'job_id': self.job_dev.id,
            'source_id': self.richard_emp_2.utm_source_id.id
        })
        self.assertEqual(job_applicant.ref_user_id, self.richard_user, "Referral is created with the right user")
        ...
        self.assertEqual(2 * job_applicant.stage_id.points, sum(points_richard_c1.mapped('points')), "Right amount of referral points are created.")
    ...
    def test_create_applicant_without_referral(self):
        job_applicant = self.env['hr.applicant'].create({
            'name': 'Technical worker',
            'job_id': self.job_dev.id,
            'ref_user_id': self.user_without_employee.id
        })

        self.assertEqual(job_applicant.earned_points, 0, "No points must be created.")
