...
class GermanTaxReportTest(AccountSalesReportCommon):
    ...
    def test_generate_xml(self):
        first_tax = self.env['account.tax'].search([('name', '=', '19% sales tax'), ('company_id', '=', self.company_data['company'].id)], limit=1)
        second_tax = self.env['account.tax'].search([('name', '=', 'Intragem. Acquisition 19%USt/19%VSt'), ('company_id', '=', self.company_data['company'].id)], limit=1)
