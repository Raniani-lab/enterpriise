...
const PosBlackBoxSwedenPosGlobalState = (PosGlobalState) =>
    class PosBlackBoxSwedenPosGlobalState extends PosGlobalState {
        useBlackBoxSweden() {
            ...
            const data = {
                ...
                negative_total:
                    order.get_total_with_tax() < 0
                        ? Math.abs(order.get_total_with_tax())
                              .toFixed(2)
                              .toString()
                              .replace(".", ",")
                        : "0,00",
...
const PosBlackBoxSwedenOrder = (Order) =>
    class PosBlackBoxSwedenOrder extends Order {
        ...
        async add_product(product, options) {
            if (this.pos.useBlackBoxSweden() && product.taxes_id.length === 0) {
                ...
                this.pos.useBlackBoxSweden() &&
                this.hasNegativeAndPositiveProducts(product)
            ) {
                await Gui.showPopup("ErrorPopup", {
                    title: _t("POS error"),
...
const PosBlackBoxSwedenOrderline = (Orderline) =>
    class PosBlackBoxSwedenOrderline extends Orderline {
        export_for_printing() {
