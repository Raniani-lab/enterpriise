...
class TestForecastAccessRights(TestCommonForecast):
    ...
    def test_public_user_access_rights(self):
        ...
        with self.assertRaises(AccessError):
            self.env['project.forecast'].sudo(self.user_public.id).create({
        ...
        # read
        with self.assertRaises(AccessError):
            self.project_opera_forecast.sudo(self.user_public.id).read()
        # update
        with self.assertRaises(AccessError):
            self.project_opera_forecast.sudo(self.user_public.id).write({'resource_hours': 6})
        # delete
        with self.assertRaises(AccessError):
            self.project_opera_forecast.sudo(self.user_public.id).unlink()

    def test_portal_user_access_right(self):
        # create
        with self.assertRaises(AccessError):
            self.env['project.forecast'].sudo(self.user_portal.id).create({
                'employee_id': self.employee_bert.id,
                'start_datetime': datetime(2019, 6, 5, 8),
                'end_datetime': datetime(2019, 6, 5, 17),
                'project_id': self.project_horizon.id,
        ...
        # read
        with self.assertRaises(AccessError):
            self.project_opera_forecast.sudo(self.user_portal.id).read()
        # update
        with self.assertRaises(AccessError):
            self.project_opera_forecast.sudo(self.user_portal.id).write({'resource_hours': 6})
        # delete
        with self.assertRaises(AccessError):
            self.project_opera_forecast.sudo(self.user_portal.id).unlink()

    def test_regular_user_access_rights(self):
        # create
        with self.assertRaises(AccessError):
            self.env['project.forecast'].sudo(self.employee_joseph.user_id.id).create({
                'employee_id': self.employee_bert.id,
                'start_datetime': datetime(2019, 6, 5, 8),
                'end_datetime': datetime(2019, 6, 5, 17),
                'resource_hours': 8,
        ...
        # update
        with self.assertRaises(AccessError):
            self.project_opera_forecast.sudo(self.employee_joseph.user_id.id).write({'resource_hours': 6})
        # delete
        with self.assertRaises(AccessError):
            self.project_opera_forecast.sudo(self.employee_joseph.user_id.id).unlink()
        # joseph is not part of the tinyhouse project which is restricted to followers
        with self.assertRaises(AccessError):
            self.project_tinyhouse_forecast.sudo(self.employee_joseph.user_id).read()
        self.project_opera_forecast.sudo(self.employee_joseph.user_id).read()

    def test_manager_access_rights(self):
        # read
        self.project_tinyhouse_forecast.sudo(self.employee_bert.user_id.id).read()
        # create
        self.env['project.forecast'].sudo(self.employee_bert.user_id.id).create({
            'employee_id': self.employee_bert.id,
            'start_datetime': datetime(2019, 6, 5, 8),
            'project_id': self.project_horizon.id,
            'end_datetime': datetime(2019, 6, 5, 17),
        ...
        # update
        self.project_opera_forecast.sudo(self.employee_bert.user_id.id).write({'resource_hours': 6})
        # delete
        self.project_opera_forecast.sudo(self.employee_bert.user_id.id).unlink()
