...
class TestSubscriptionPayments(PaymentCommon, TestSubscriptionCommon):
    ...
    def test_04_auto_payment_with_token(self):
        ...
        self.assertEqual(
            invoice.amount_total, recurring_total_with_taxes,
        ...
        self.assertTrue(
            all(tax_line.tax_line_id == self.tax_10 for tax_line in invoice.line_ids.filtered(
                'tax_line_id'
            )),
        ...
        for dummy in range(5):
            subscription_mail_fail.copy(
                {'to_renew': True, 'stage_id': self.subscription.stage_id.id}
            )

        self.env['sale.subscription'].with_context(auto_commit=False)._recurring_create_invoice(
            automatic=True, batch_size=3
        )
