...
class TestSubscriptionController(PaymentCommon, TestSubscriptionCommon, HttpCase):
    def setUp(self):
        super().setUp()
    ...
    def test_prevents_assigning_not_owned_payment_tokens_to_subscriptions(self):
        ...
        data = {'token_id': legit_payment_method.id, 'order_id': legit_user_subscription.id}
        payload = self._build_payload(data)
        headers = {"Content-Type": "application/json"}
        url = "/my/subscription/assign_token/%s" % legit_user_subscription.id
        res = self.url_open(url, allow_redirects=False, data=json.dumps(payload), headers=headers)
        ...
        payload = self._build_payload(data)
        url = "/my/subscription/assign_token/%s" % malicious_user_subscription.id
        res = self.url_open(url, allow_redirects=False, data=json.dumps(payload), headers=headers)
        self.assertEqual(res.status_code, 200)
