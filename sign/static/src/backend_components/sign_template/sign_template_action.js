...
export class SignTemplate extends Component {
    setup() {
        ...
        onWillStart(async () => {
            ...
            return Promise.all([
                this.checkManageTemplateAccess(),
    ...
    async fetchTemplateData() {
        const template = await this.orm.call(
            "sign.template",
            "read",
            [[this.templateID], ['id', 'attachment_id', 'has_sign_requests', 'responsible_count', 'display_name']],
        );
        if (!template.length) {
            this.templateID = undefined;
            this.notification.add(
                this.env._t("The template doesn't exist anymore."), 
                {
    ...
    async fetchSignRoles() {
        this.signRoles = await this.orm.call(
            "sign.item.role",
            "search_read",
            [],
            { context: this.user.context },
