...
const EditableDocumentBackend = DocumentBackend.extend({
    events: {
        'click .o_sign_resend_access_button': function(e) {
            ...
            }).then(() => {$envelope.empty().append(_t("Resent !"))});
        },
    },
    ...
    start: function() {
        ...
        if (nextTemplate && nextTemplate.template) {
            const nextDocumentButton = $('<button/>', {html: _t("Next Document"), type: "button", 'class': 'btn btn-primary mr-2'});
            ...
            if (this.cp_content) {
                this.cp_content.$buttons = nextDocumentButton.add(this.cp_content.$buttons);
            }
        }

        return this._super.apply(this, arguments).then(() => {
            if(this.is_author && this.is_sent) {
                this.$('.o_sign_signer_status').not('.o_sign_signer_signed').each((i, el) => {
                    $(el).append($('<button/>', {
                        type: 'button',
                        title: (this.requestStates && this.requestStates[el.dataset.id]) ? _t("Resend the invitation"): _t("Send the invitation"),
                        text: (this.requestStates && this.requestStates[el.dataset.id]) ? _t("Resend"): _t("Send"),
