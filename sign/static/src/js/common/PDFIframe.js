...
export const PDFIframe = Widget.extend(Object.assign({}, PinchItemMixin, {
    ...
    start: function () {
        ...
        let viewerURL = "/web/static/lib/pdfjs/web/viewer.html?unique="+ (+new Date()) +"&file=";
        viewerURL += encodeURIComponent(self.attachmentLocation).replace(/'/g,"%27").replace(/"/g,"%22") + "#page=1";
    ...
    display_select_options: function ($container, options, selected_options, readonly, active_option) {
        ...
        selected_options.forEach(function (id, index) {
            ...
            const $op = $('<span class="o_sign_item_option"/>').text(options[id].value);
            $op.data('id', id);
            $container.append($op);
            if (!readonly) {
        ...
        function select_option($container, option_id) {
            const $selected_op = $container.find(':data(id)').filter(function () { return $(this).data('id') === option_id;});
            const $other_options = $container.find(':data(id)').filter(function () { return $(this).data('id') !== option_id;});
    ...
    createSignItem: function (type, required, responsible, posX, posY, width, height, value, option_ids, name, tooltip, alignment, isSignItemEditable) {
        ...
        const $signatureItem = $(core.qweb.render('sign.sign_item', {
