...
QUnit.module("spreadsheet_edition > model_selector", { beforeEach });

QUnit.test("with no model", async function (assert) {
...
QUnit.test("displays model display names", async function (assert) {
...
QUnit.test("with 8 models", async function (assert) {
    await mountModelSelector([
        "model_1",
        "model_2",
        "model_3",
...
QUnit.test("with more than 8 models", async function (assert) {
    await mountModelSelector([
        "model_1",
        "model_2",
        "model_3",
...
QUnit.test("search content is not applied when opening the autocomplete", async function (assert) {
    await mountModelSelector(["model_1", "model_2"], "_2");
    await openAutocomplete();
    assert.containsN(fixture, "li.o-autocomplete--dropdown-item", 2);
});

QUnit.test("with search matching some records on technical name", async function (assert) {
...
QUnit.test("with search matching some records on business name", async function (assert) {
    await mountModelSelector(["model_1", "model_2"]);
    await openAutocomplete();
...
QUnit.test("with search matching no record", async function (assert) {
    await mountModelSelector(["model_1", "model_2"]);
...
QUnit.test("select a model", async function (assert) {
    ...
    await openAutocomplete();
    await click(fixture.querySelector(".o_sp_selector_model_2"));
    assert.verifySteps(["model selected"]);
});

QUnit.test("with an initial value", async function (assert) {
