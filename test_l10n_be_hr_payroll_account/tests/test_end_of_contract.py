...
@tagged('post_install', '-at_install', 'end_of_contract_no_public_leave_right')
class TestEndOfContractNoPublicLeaveRight(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref='l10n_be.l10nbe_chart_template'):
        ...
        cls.car = cls.env['fleet.vehicle'].create([{
            ...
            'company_id': cls.env.company.id,
            'model_id': cls.model.id,
            'first_contract_date': datetime.date(2020, 11, 20),
            'co2': 88.0,
            'car_value': 38000.0,
            'fuel_type': "diesel",
            'acquisition_date': datetime.date(2020, 1, 1)
        ...
        cls.contracts = cls.env['fleet.vehicle.log.contract'].create([{
            ...
            'expiration_date': datetime.date(2021, 11, 20),
            'state': "open",
            'cost_generated': 0.0,
            'cost_frequency': "monthly",
            'recurring_cost_amount_depreciated': 0.0
        ...
        cls.contract = cls.env['hr.contract'].create([{
            ...
            'structure_type_id': cls.env.ref('hr_contract.structure_type_employee_cp200').id,
            'date_start': datetime.date(2018, 12, 31),
            'date_end': datetime.date(2020, 10, 13),
            'wage': 2650.0,
            'state': "open",
            'hourly_wage': 0.0,
            'holidays': 0.0,
    ...
    def test_end_of_contract_no_public_leave_right(self):
        ...
        work_entries = self.contract._generate_work_entries(datetime.date(2020, 10, 1), datetime.date(2020, 10, 31))
        work_entries.action_validate()
        self.payslip._onchange_employee()
        self.payslip.compute_sheet()
        ...
        self.assertAlmostEqual(self.payslip._get_worked_days_line_amount('LEAVE510'), 48.92, places=2)
        self.assertAlmostEqual(self.payslip._get_worked_days_line_number_of_days('LEAVE510'), 1.0, places=2)
        self.assertAlmostEqual(self.payslip._get_worked_days_line_number_of_hours('LEAVE510'), 7.6, places=2)
        ...
        self.assertAlmostEqual(self.payslip._get_worked_days_line_amount('LEAVE510'), 0, places=2)
        self.assertAlmostEqual(self.payslip._get_worked_days_line_number_of_days('LEAVE510'), 0.0, places=2)
        self.assertAlmostEqual(self.payslip._get_worked_days_line_number_of_hours('LEAVE510'), 0.0, places=2)
