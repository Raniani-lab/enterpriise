...
class VoipPhonecall(models.Model):
    ...
    sequence = fields.Integer(
        "Sequence", index=True, help="Gives the sequence order when displaying a list of Phonecalls."
    )
    ...
    direction = fields.Selection([("incoming", "Incoming"), ("outgoing", "Outgoing")], default="outgoing")
    ...
    def hangup_call(self, duration_seconds=0, done=True):
        ...
        if done:
            ...
            if self.activity_id:
                ...
                duration_log = Markup("<br/><p>%s</p>") % _(
                    "Call duration: %(min)smin %(sec)ssec", min=minutes, sec=seconds
                )
    ...
    def get_next_activities_list(self):
        return (
            self.search(
                ...
                order="sequence,date_deadline,id",
            )
