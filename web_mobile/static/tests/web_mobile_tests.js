        ...
        function () {
            QUnit.test("contact sync in a non-mobile environment", async function (assert) {
                ...
                assert.strictEqual(
                    $button.length,
                    0,
                    "the tag should not be visible in a non-mobile environment"
                );
                assert.strictEqual(
                    rpcCount,
                    1,
                    "no extra rpc should be done by the widget (only the one from the view)"
                );
            ...
            QUnit.test(
                ...
                async function (assert) {
                    ...
                    assert.strictEqual(
                        window.scrollY,
                        scrollPosition.top,
                        "Should have scrolled 265 px vertically"
                    );
                    assert.strictEqual(
                        window.scrollX,
                        scrollPosition.left,
                        "Should be 0 px from left as it is"
                    );
                    // click on m2o field
            ...
            QUnit.test("contact sync in a mobile environment", async function (assert) {
                ...
                assert.strictEqual(
                    $button.length,
                    1,
                    "the tag should be visible in a mobile environment"
                );
                assert.strictEqual(
                    rpcCount,
                    1,
            ...
            QUnit.test("autofocus quick create form", async function (assert) {
                ...
                assert.ok(
                    kanban
                        .$(".o_kanban_group:nth(0) > div:nth(1)")
                        .hasClass("o_kanban_quick_create"),
                    "clicking on create should open the quick_create in the first column"
                );
                assert.strictEqual(
                    document.activeElement,
                    kanban.$(".o_kanban_quick_create .o_input:first")[0],
